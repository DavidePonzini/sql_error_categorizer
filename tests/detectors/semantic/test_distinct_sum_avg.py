...
def test_distinct_in_sum():
    ...
    assert has_error(result, SqlErrors.SEM_1_INCONSISTENT_EXPRESSION_DISTINCT_IN_SUM_OR_AVG, ('SUM(DISTINCT 1 + order_id)',))
...
def test_no_distinct_in_sum():
    ...
    assert count_errors(result, SqlErrors.SEM_1_INCONSISTENT_EXPRESSION_DISTINCT_IN_SUM_OR_AVG) == 0
